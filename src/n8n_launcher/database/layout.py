"""Database schema layout detection — migration files and schema declaration."""
...
def migrate_files(migrations_dir: Path) -> list[Path]:
    """Return the sorted ``.sql`` files directly inside ``migrations_dir``."""
...
def detect_migrations(workflows_dir: Path) -> list[Path]:
    ...
    return migrate_files(workflows_dir / "db" / "migrations")
