"""GitHub REST API helpers used to create repositories from the launcher."""
...
class GitHubClient:
    ...
    Used exclusively for read/validate (:meth:`github_owner`) and repository
    creation (:meth:`create_repo`). The token is never persisted by this class
    nor by its callers; errors are wrapped in :class:`GitHubError` with the
    HTTP status preserved so the GUI can explain 401/403/422 to the user.
    """
    ...
    def create_repo(
        ...
        *,
        ...
        The returned ``https://github.com/<owner>/<name>.git`` URL is meant to
        be stored as the git remote; it contains no token.
        """
