...
def prompt_db_config(root: tk.Tk, current: DbConfig) -> DbConfig | None:
    ...
    Returns the chosen :class:`DbConfig` or ``None`` when cancelled. Password
    fields left empty are regenerated by the manager on save.
    """
    dialog = tk.Toplevel(root)
    ...
    db_var, db_name_entry = field("Base de données :", current.database_name or "data")
    user_var, _ = field("Utilisateur :", current.username or "n8ndata")
    pass_var, _ = field("Mot de passe :", current.password or "")
