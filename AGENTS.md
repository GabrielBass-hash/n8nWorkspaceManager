...
- **Hook protocol**: the generated `post-receive` hook runs `docker compose -p "$PROJECT" up -d` (matching `render_deploy_script`'s `PROJECT = os.environ["DEPLOY_PROJECT"]`), executes `deploy.py`, and records the result in a marker file `last-deploy.json` (`{sha,status,error,at}`) written **atomically** (`.tmp` + `os.replace`). Content is a template string — never an f-string.
