...
from n8n_launcher.updater import (
    REPO,
    Asset,
    Release,
    UpdateError,
...
def test_install_target_macos_finds_app_bundle(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "n8n_launcher.updater.platform.system", lambda: "Darwin"
    )
    monkeypatch.setattr(
        "sys.executable",
...
def test_install_target_other_platforms_return_executable(monkeypatch) -> None:
    monkeypatch.setattr("sys.frozen", True, raising=False)
    monkeypatch.setattr(
        "n8n_launcher.updater.platform.system", lambda: "Windows"
    )
...
def test_installer_script_writes_executable_sh(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "n8n_launcher.updater.platform.system", lambda: "Linux"
    )
...
def test_spawn_installer_launches_detached(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("n8n_launcher.updater.subprocess.Popen", mock_popen)
