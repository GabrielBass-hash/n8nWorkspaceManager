...
def reset_font_state():
    """Each test registers fonts against a fresh fake root."""
    theme._FONTS_CONFIGURED = False
    yield
    theme._FONTS_CONFIGURED = False
...
def test_configure_fonts_registers_all_fonts_with_scanned_family() -> None:
    ...
    with patch("tkinter.font.families", return_value=families) as probe:
...
def test_configure_fonts_prefers_first_available_and_ignores_case() -> None:
    ...
    with patch("tkinter.font.families", return_value=families):
...
def test_configure_fonts_falls_back_to_tkdefaultfont() -> None:
    ...
    with patch("tkinter.font.families", return_value=["fixed"]):
        assert theme.configure_fonts(root) is True
...
def test_configure_fonts_is_idempotent() -> None:
    tk = MagicMock()
    root = SimpleNamespace(tk=tk)

    with patch("tkinter.font.families", return_value=["liberation sans"]):
        assert theme.configure_fonts(root) is True
        tk.call.reset_mock()
        assert theme.configure_fonts(root) is True
